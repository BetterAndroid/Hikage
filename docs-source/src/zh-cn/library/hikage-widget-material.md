...
这是 Hikage 针对 Google Material (MDC) 组件的声明依赖。
...
这个依赖中声明了来自 Google Material (MDC) 中的可用组件，你可以直接引用它们到 Hikage 中使用。

你需要手动引入 `com.google.android.material:material` 依赖，这个依赖不提供 Material 相关依赖的自动引入。
